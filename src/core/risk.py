...
class RiskEngine:
    ...
    Operates strictly in UTC timezone.
    """
    ...
    def is_kill_switch_active(self) -> bool:
        """Check if emergency kill switch is active via environment variable."""
    ...
    def evaluate_order(
        ...
        Returns: RiskDecision(approved=..., reason=..., adjusted_volume=...)
        """
        ...
        # 3. Maximum Daily Loss Gate (5% drawdown from daily starting balance)
        if daily_starting_balance > 0:
            daily_loss_pct = daily_pnl / daily_starting_balance
        ...
        # 4. Maximum Concurrent Positions Gate (3 total)
        ...
        # 5. Maximum Exposure per Symbol (1 position per symbol)
        ...
        # 7. Dynamic Position Sizing Calculation
        ...
        return RiskDecision(
            approved=True,
            reason=f"All risk gates passed. Allocated volume: {volume:.2f} lots (Risk: ${equity * self.config.max_risk_per_trade_pct:.2f})",
